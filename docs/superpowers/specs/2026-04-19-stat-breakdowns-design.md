...
## 4. Экипированное оружие в InventoryPanel

**Переключатель:** у каждого предмета с тегом `weapon` в `InventoryPanel.vue` — иконка-кнопка ⚔ (или небольшой чекбокс) для toggle `item.equipped`. Неоружейные предметы переключателя не получают.

**Варнинг:** если экипировано >1 оружия — статичный блок вверху `InventoryPanel`:
```
